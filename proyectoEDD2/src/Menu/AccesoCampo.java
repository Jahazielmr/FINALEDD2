...
public class AccesoCampo {
    ...
    public void guardarIndices(ArrayList<Integer> indices) throws IOException{
        int espacio = 20;
        flujo.seek(0);
        flujo.writeUTF(""+indices.size());
        for (int i = 1; i < indices.size()+1; i++) {
            flujo.seek(i*espacio);
            flujo.writeUTF(indices.get(i)+"");
        }
    }
    
    public ArrayList<Integer> cargarIndices() throws IOException{
        int espacio = 20;
        ArrayList<Integer> temp = new ArrayList();
        flujo.seek(0);
        int cantidadIndices = Integer.parseInt(flujo.readUTF());
        for (int i = 1; i < cantidadIndices+1; i++) {
            flujo.seek(i*espacio);
